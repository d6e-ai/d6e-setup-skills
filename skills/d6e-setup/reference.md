...
| `ORIGIN`                     | Yes      | -       | Public URL of D6E instance (e.g. `https://example.d6e.ai`). Used by the built-in SvelteKit console; `${ORIGIN}/auth/callback` should be registered on d6e-auth when the instance client is created. Custom frontend callback URLs are also registered on d6e-auth (franchise portal or per-workspace Redirect URIs), not via instance env vars. |
