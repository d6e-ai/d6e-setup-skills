...
| Variable                 | Required | Default     | Description                                                        |
| ------------------------ | -------- | ----------- | ------------------------------------------------------------------ |
| `D6E_AUTH_URL`           | Yes      | -           | d6e-auth server URL (e.g. `https://auth.d6e.ai`)                  |
...
5. Set these in your `.env` file along with the `D6E_AUTH_URL` provided by the administrator
...
| `compose.withdb.yml`| Development / local (includes DB)     | Builds from source, includes PostgreSQL container  |
